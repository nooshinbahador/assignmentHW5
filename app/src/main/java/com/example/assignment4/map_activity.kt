...
class map_activity : AppCompatActivity(),OnMapReadyCallback {
    ...
    private fun createGeofence(selectedLocation:LatLng,reminder: Reminder,geofencingClient: GeofencingClient){
        val geofence = Geofence.Builder().setRequestId(GEOFENCE_ID)
            .setCircularRegion(selectedLocation.latitude,selectedLocation.longitude,
                GEOFENCE_RADIUS.toFloat()).setExpirationDuration(GEOFENCE_EXPRIRATION.toLong())
        ...
        val intent = Intent(this,GeofenceReceiver::class.java)
            .putExtra("uid",reminder.uid).putExtra("message",reminder.message)
    ...
    override fun onMapReady(map: GoogleMap?) {
        ...
        gMap.setOnMapClickListener { location:LatLng ->
            with(gMap){
                ...
                addCircle(CircleOptions().center(location).strokeColor(Color.argb(50,70,70,70))
